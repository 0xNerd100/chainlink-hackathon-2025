...
const TOKEN_ADDRESSES = {
  ...
  avalancheFuji: {
    ...
    RWAL: "0x607ef39311FBe999ae61DfC2656742b5e2FB77dF"
  },
  bscTestnet: {
    USDL: "0x33217BE7c92DA5A0F2151c13974f14236cbebaa3",
    RWAL: "0x4DF8f1dc4aD0DeECaA0A6CeaC04B26E577D93996"
  }
};
...
const TOKEN_MAPPING = {
  ...
  avalancheFuji: {
    ...
    RWAL: { bscTestnet: "RWAL" }
  },
  bscTestnet: {
    USDL: { avalancheFuji: "USDL" },
    RWAL: { avalancheFuji: "RWAL" }
  }
};
...
export const useBridgeHelper = () => {
  ...
  const getTokenBalance = useCallback(async (tokenAddress: string, userAddress: string, networkConfig: any): Promise<TokenBalance> => {
    if (!publicClient) {
  ...
  const estimateFee = useCallback(async (params: BridgeParams): Promise<bigint> => {
  ...
  const approveToken = useCallback(async (tokenAddress: string, spenderAddress: string, amount: bigint): Promise<`0x${string}`> => {
  ...
  const bridgeTokens = useCallback(async (params: BridgeParams): Promise<{ hash: `0x${string}`, messageId: string }> => {
  ...
  }, [walletClient, publicClient, address, checkAllowance, approveToken, estimateFee, encodeExtraArgs, getTokenBalance, isValidAddress, log, logError]);
